...
train_bond = np.load('train_nr-ahr.npy.zip')['train_nr-ahr.npy'].item()
...
for fold in  range(0,2):
    ...
    train_id = [x for x in range(len(all_id)) if x%2 != fold]
    val_id =  [x for x in range(len(all_id)) if x%2== fold]
    for epoch in range(30):
        ...
        if True:
            ...
            if epoch >= 2:
                ...
                test = test.set_value(range(len(test)),
                                                      'predict1',
                                                      test['predict1'] + np.array(test_pred)/2)
                #save_path = saver.save(sess,'./model300_reweigh_loss_%s_%s.ckpt' %(epoch,int(100*np.mean(val_cost))))
        ...
        save_path = saver.save(sess,'./model2300_reweigh_loss_%s_%s.ckpt' %(fold,epoch))
