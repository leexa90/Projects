...
def make_info(x,num=None):
    ...
    dist =  Chem.GetDistanceMatrix(mol)* (Chem.GetDistanceMatrix(mol)<1000)
    ...
    charges = [float(mol.GetAtomWithIdx(x).GetProp('_GasteigerCharge')) for x in range(len(adj))]
    temp = np.concatenate([np.stack([adj,dist],-1),bond_order],-1)
    if num == None:
        return  [mol,temp.astype(np.float32),atoms,mol.GetPropsAsDict(),charges]
    else:
        return [mol,temp.astype(np.float32),atoms,mol.GetPropsAsDict(),charges][num]

train['atoms']= train['SMILES'].apply(lambda x : make_info(x,2))
train['bond']= train['SMILES'].apply(lambda x : make_info(x,1)[:,:,-3:])
train['charges'] = train['SMILES'].apply(lambda x : make_info(x,4))
...
train['SMILES'] = train['SMILES'].apply(lambda x : make_info(x,0))
...
del train['charges']
train.to_csv('permability_features.csv',index=0)
