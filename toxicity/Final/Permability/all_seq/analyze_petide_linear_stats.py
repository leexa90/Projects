...
for feats in ['SMR_VSA','PEOE_VSA','SlogP_VSA']:
    ...
    for feat in feat_all:
        train[feat+'_norm']=train[feat]/np.sum(train[feat_all],1)
...
if True:
    ...
    features = ['charge_skew', 'charge_kurtosis', 'charge_std', 'atom_8', 'atom_sum_8', 'atom_1',
                'atom_sum_1', 'atom_16', 'atom_sum_16', 'atom_6', 'atom_sum_6', 'atom_7', 'atom_sum_7',
                'TPSA', 'TPSA_norm', 'LabuteASA', 'LabuteASA_norm', 'PEOE_VSA1', 'PEOE_VSA2', 'PEOE_VSA3',
                'PEOE_VSA4', 'PEOE_VSA5', 'PEOE_VSA6', 'PEOE_VSA7', 'PEOE_VSA8', 'PEOE_VSA9', 'PEOE_VSA10',
                'PEOE_VSA11', 'PEOE_VSA12', 'PEOE_VSA13', 'PEOE_VSA14', 'SMR_VSA1', 'SMR_VSA2', 'SMR_VSA3',
                ...
                'SlogP_VSA7', 'SlogP_VSA8', 'SlogP_VSA9', 'SlogP_VSA10', 'SlogP_VSA11', 'VSA_EState1',
                'VSA_EState2', 'VSA_EState3', 'VSA_EState4', 'VSA_EState5', 'VSA_EState6', 'VSA_EState7',
                'VSA_EState8', 'VSA_EState9', 'VSA_EState10', 'HBA', 'HBD', 'Rotatable_num', 'HBA_norm',
                'HBD_norm', 'atom_len', 'atom_size', 'SMR_VSA1_norm', 'SMR_VSA2_norm',
                'SMR_VSA3_norm', 'SMR_VSA4_norm', 'SMR_VSA5_norm', 'SMR_VSA6_norm', 'SMR_VSA7_norm',
                'SMR_VSA8_norm', 'SMR_VSA9_norm', 'SMR_VSA10_norm', 'PEOE_VSA1_norm', 'PEOE_VSA2_norm',
                'PEOE_VSA3_norm', 'PEOE_VSA4_norm', 'PEOE_VSA5_norm', 'PEOE_VSA6_norm', 'PEOE_VSA7_norm',
                'PEOE_VSA8_norm', 'PEOE_VSA9_norm', 'PEOE_VSA10_norm', 'PEOE_VSA11_norm', 'PEOE_VSA12_norm',
                'PEOE_VSA13_norm', 'PEOE_VSA14_norm', 'SlogP_VSA1_norm', 'SlogP_VSA2_norm', 'SlogP_VSA3_norm',
                'SlogP_VSA4_norm', 'SlogP_VSA5_norm', 'SlogP_VSA6_norm', 'SlogP_VSA7_norm', 'SlogP_VSA8_norm',
                'SlogP_VSA9_norm', 'SlogP_VSA10_norm', 'SlogP_VSA11_norm']
...
for seed in range(0,100):
    ...
    test1=test.sort_values('10').iloc[0::2].reset_index(drop=True)
    test1 = test1.sample(len(test)//2,replace=replace, random_state=seed).reset_index(drop=True)
    test2=test.sort_values('10').iloc[1::2].reset_index(drop=True)
    
    train1=train.sort_values(target).iloc[0::2].reset_index(drop=True).reset_index(drop=True)
    train1 = train1.sample(len(train)//2,replace=replace, random_state=seed).reset_index(drop=True)
    train2=train.sort_values(target).iloc[1::2].reset_index(drop=True)
    for alpha in (0.0001,0.0003,0.001,0.003,0.01,0.03,0.1,0.3)[3:4]:
        X,Y1,Y2 = [],[],[]
...
die
if True:
        test['10a']  = np.log10(test['10'])
        features = [x[0] for x in results]+['10a',]
        corr = np.abs(test[features].corr().values)
        D = corr
        ...
        z=plt.imshow(D)
        plt.legend()
        plt.xticks(range(len(index)),sorted_keys,rotation='vertical', fontsize=6)
        plt.yticks(range(len(index)),sorted_keys, fontsize=6)
        plt.colorbar(z)
        plt.savefig('Corr53.png',dpi=300,bbox_inches='tight')
        plt.close()
        Z = sch.dendrogram(Y,orientation='right')
        plt.yticks(range(6,6+len(index)*10,10),sorted_keys, fontsize=6)
        plt.savefig('cluster_53feat.png',dpi=500)
        plt.show()
if True:
    results = []
...
def main():
    ...
    dataC=[]
    dataD=[]
    for alpha in [0,]:#(0.0001,0.0003,0.001,0.003,0.01,0.03,0.1,0.3):
        X,Y1,Y2 = [],[],[]
        from sklearn import linear_model,preprocessing ,decomposition
        from sklearn.decomposition import PCA
        from sklearn.preprocessing import StandardScaler
        ...
        print D[D['a']!=0].sort_values('a').values
        r1 = 1-np.mean((10**preds -test1['10'])**2)/np.mean((np.mean(test1['10']) -test1['10'])**2)
        r1 = str(np.corrcoef(preds ,np.log10(test1['10']))[0,1])[:4]
        r2 = str(np.corrcoef(preds2 ,np.log10(test2['10']))[0,1])[:4]#str(np.mean((preds2 -np.log10(test2['10']))**2)**.5)[:4]#str(1-np.mean((10**preds2 -test2['10'])**2)/np.mean((np.mean(test2['10']) -test2['10'])**2))[:4]
        dataA += [r2,]
        print r1,r2
        print alpha,a,b,r1,r2
        if plots:
            ...
            plt.title('Stapled Peptide model predict stapled peptide\ncorr coef = %s'%r2);
            plt.xlabel('predictions of log flourescence');plt.ylabel('ground truth of log flourescence');
            plt.plot(preds2,np.log10(test2['10']),'bo',label='test predictions');plt.legend();plt.show()
...
if True:
    ...
    counter = 1
    for i in range(len(bins)-1):
        print counter,bins[i:i+2], WC[(WC['Log P'] > bins[i])&(WC['Log P'] <= bins[i+1])]['descriptions'].values,
        print WC[(WC['Log P'] > bins[i])&(WC['Log P'] <= bins[i+1])]['SMARTSa'].values
        counter += 1
