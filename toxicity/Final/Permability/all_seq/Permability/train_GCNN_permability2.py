...
data = pd.read_csv('permability.csv')
data = data[data['set'] !='val']
testId = np.array([   5,   36,   56,   57,  104,  115,  255,  301,  322,  329,  394,
        406,  416,  418,  419,  424,  430,  439,  455,  465,  469,  472,
        503,  519,  521,  526,  538,  545,  547,  558,  566,  653,  654,
        676,  724,  749,  750,  751,  758,  760,  775,  814,  844,  859,
        868,  869,  870,  915,  927,  928,  944,  946,  950,  951,  952,
       1007, 1012, 1013, 1015, 1049, 1069, 1090, 1104, 1109, 1137, 1188,
       1226, 1229, 1257,    7,   52,   69,  113,  149,  150,  153,  163,
        189,  192,  213,  222,  223,  229,  243,  246,  262,  263,  296,
        302,  308,  309,  330,  331,  334,  335,  344,  353,  358,  384,
        386,  390,  412,  422,  427,  428,  431,  434,  435,  438,  445,
        448,  451,  452,  482,  484,  485,  489,  504,  518,  542,  543,
        544,  549,  553,  554,  556,  557,  563,  600,  605,  617,  649,
        689,  691,  718,  756,  849,  856,  877,  901,  902,  916,  945,
        947,  949,  960,  979,  980, 1028, 1072, 1085, 1117, 1121, 1127,
       1131, 1138, 1166, 1175, 1177, 1182, 1199, 1217, 1241, 1246, 1260,
          4,   15,   37,   47,   48,   59,   99,  169,  175,  176,  206,
        212,  221,  293,  352,  364,  366,  379,  398,  410,  420,  444,
        453,  456,  459,  461,  471,  494,  513,  530,  535,  552,  601,
        602,  613,  655,  673,  674,  687,  709,  710,  744,  745,  746,
        754,  762,  803,  846,  871,  977,  981, 1014, 1038, 1075, 1084,
       1093, 1105, 1115, 1134, 1162, 1170, 1200, 1266,   45,  110,  116,
        137,  152,  184,  196,  225,  226,  261,  285,  286,  291,  303,
        336,  337,  338,  339,  340,  342,  345,  365,  382,  387,  397,
        421,  476])
data['set'] = 'Tr'
data = data.set_value(testId,'set','Te')

train_bond = np.load('permability.npy.zip')['permability.npy'].item()
test_bond = np.load('permability.npy.zip')['permability.npy'].item()
chal_bond = np.load('permability.npy.zip')['permability.npy'].item()

train = data.copy(deep=True)
train = train[train['set']=='Tr'].reset_index(drop=True) #only keep training set

chal = data.copy(deep=True)
chal = chal[chal['set']=='Te'].reset_index(drop=True) #only keep val set

test = pd.read_csv('permability.csv')
test = test[test['set']=='val'].reset_index(drop=True) #only keep test set
...
def weave_final(A_matrix,P_matrix,
    ...
    P0 = P_P (P_matrix,name+'_P0_',activation=activation,depth=64)
    P1 = A_P(A_matrix,num_atoms,name+'_P1_',activation=activation,depth=64)
    A0 = A_A (A_matrix,name+'_A0_',activation=activation,depth=64)
    A1 = P_A(P_matrix,name+'_A1_',Adj_matrix=Adj_matrix,activation=activation,depth=64)
    P2 = tf.concat([P0,P1],-1,name+'_P2_')
    A2 = tf.concat([A0,A1],-1,name+'_A2_')
    activation = None #final convolution
    P3 = P_P(P2,name+'_P3_',activation=activation,depth=64)
    A3 = A_A(A2,name+'_A3_',activation=activation,depth=64)
    return A3,P3
### WEAVE FUNCTION END###
...
dense5 = dense5a #tf.nn.sigmoid(dense5a)
# Evaluation
...
if len(saved_files) >= 1:
    ...
    saver.restore(sess,'model_final_reweigh_loss_0_61.ckpt')#saver.restore(sess,'./'+last_file)
...
for fold in  range(0,5):
    #if fold != 0: 
    saver.restore(sess,'model_final_reweigh_loss_%s_61.ckpt' %fold)
...
##            return np.stack(x,0)
        ...
        if True:
            ...
            for sample in train_id:
                ...
                train_pred += [out[0][0],]
                train_gd += [y,]
            print (len(train_cost),np.mean(train_cost),R2_score(train_gd,train_pred))
            val_pred = []
            val_gd = []
            for sample in val_id:
                ...
                val_pred += [out[0][0],]
                val_gd += [y,]
            print (len(val_cost),np.mean(val_cost),R2_score(val_gd,val_pred))
            test_pred = []
            test_gd = []
            test_cost = []
            for sample in range(len(test)):
                ...
                test_pred += [out[0][0],]
                test_gd += [y,]
            print (len(test_cost),np.mean(test_cost),R2_score(test_gd,test_pred))
            chal_pred = []
            chal_gd = []
            chal_cost = []
            for sample in range(len(chal)):
                ...
                chal_pred += [out[0][0],]
                chal_gd += [y,]
            print (len(chal_cost),np.mean(chal_cost),R2_score(chal_gd,chal_pred))
            if epoch >= 0:
                train = train.set_value(val_id,
                                                        'predict1',
                                                        train['predict1'].iloc[val_id] + np.array(val_pred)/5)
                test = test.set_value(range(len(test)),
                                                      'predict1',
                                                      test['predict1'] + np.array(test_pred)/25)
                chal = chal.set_value(range(len(chal)),
                                                      'predict1',
                                                      chal['predict1'] + np.array(chal_pred)/25)
                print ('ENSEMBLE',len(test_cost),R2_score(test[target1],test['predict1']))
                print ('ENSEMBLE',len(chal_cost),R2_score(chal[target1],chal['predict1']))
                print '\n'

                train.to_csv('train_CNN.csv',index=0)
                test.to_csv('test_CNN.csv', index=0)
                chal.to_csv('chal_CNN.csv',index=0)
