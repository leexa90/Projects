...
train['mask'] = map(lambda x : (x>=0)*1 , train[target].values) 
chal['mask'] = map(lambda x : (x>=0)*1 , chal[target].values)
test['mask'] = map(lambda x : (x>=0)*1 , test[target].values)
train[target] = train[target].fillna(-1)
test[target] = test[target].fillna(-1)
chal[target] = chal[target].fillna(-1)
...
dense5a= tf.layers.dense(dense4,12,None)
# Evaluation
answer = tf.placeholder(tf.float32,[None,12],name='answer')
mask = tf.placeholder(tf.float32,[None,12],name='mask')
# output
dense5 = tf.multiply(tf.nn.sigmoid(dense5a),mask)
#cost = tf.reduce_mean(tf.squared_difference(dense5,answer )) #linear reg
sigmoid_cost = tf.nn.sigmoid_cross_entropy_with_logits(logits = dense5a, labels = answer )
cost =tf.reduce_mean(tf.multiply(sigmoid_cost,mask))
...
import sklearn
z=(1-train[target].apply(np.mean))/train[target].apply(np.mean)
print z
def get_weight(x): #average class weight of the molecule (highly skewed classes get higher weight)
    return int((19+np.sum((x>=1)*np.array(z)))//len(z)) 
print 'Imbalanced',z
...
##            return np.stack(x,0)
        ...
        if True: #not training
            for i in  range(0,len(shuffle),1):
                ...
                x5 = np.reshape(train.iloc[sample]['mask'].values[0],(len(x3),12))
                y = np.reshape(train.iloc[sample][target].values[0],(len(x3),12))
        ...
        if True:
            ...
            for sample in train_id:
                ...
                x4,y = len(x1),train.iloc[sample][target].values
                x5 = train.iloc[sample]['mask']
                ...
                train_pred += [out[0],]
                train_gd += [y,]
            train_gd = np.stack(train_gd,0).astype(np.float32)
            train_pred = np.stack(train_pred,0).astype(np.float32)
            print len(train_cost),np.mean(train_cost),
            for i in range(12):
                AA = train_gd[:,i] 
                BB = train_pred[:,i]
                print sklearn.metrics.roc_auc_score(AA[AA != -1],BB[AA != -1]),
            print '\n'
            val_pred = []
            val_gd = []
            for sample in val_id:
                ...
                val_pred += [out[0],]
                val_gd += [y,]
            val_gd = np.stack(val_gd,0).astype(np.float32)
            val_pred = np.stack(val_pred,0).astype(np.float32)
            print len(val_cost),np.mean(val_cost),
            for i in range(12):
                AA = val_gd[:,i] 
                BB = val_pred[:,i]
                print sklearn.metrics.roc_auc_score(AA[AA != -1],BB[AA != -1]),
            ...
            for sample in range(len(test)):
                ...
                x4,y = len(x1),test.iloc[sample][target].values
                x5 = test.iloc[sample]['mask']
                out, c = sess.run([dense5, cost], feed_dict={Adj_matrix: [x1],
                                                              A_matrix: [x2],
                                                              P_matrix: [x3],
                                                              num_atoms: x4,
                ...
                test_pred += [out[0],]
                test_gd += [y,]
            test_gd = np.stack(test_gd,0).astype(np.float32)
            test_pred = np.stack(test_pred,0).astype(np.float32)
            print len(test_cost),np.mean(test_cost),
            for i in range(12):
                AA = test_gd[:,i] 
                BB = test_pred[:,i]
                print sklearn.metrics.roc_auc_score(AA[AA != -1],BB[AA != -1]),
            ...
            for sample in range(len(chal)):
                ...
                x4,y = len(x1),chal.iloc[sample][target].values
                x5 = chal.iloc[sample]['mask']
                out, c = sess.run([dense5, cost], feed_dict={Adj_matrix: [x1],
                                                              A_matrix: [x2],
                                                              P_matrix: [x3],
                                                              num_atoms: x4,
                ...
                chal_pred += [out[0],]
                chal_gd += [y,]
            chal_gd = np.stack(chal_gd,0).astype(np.float32)
            chal_pred = np.stack(chal_pred,0).astype(np.float32)
            print len(chal_cost),np.mean(chal_cost),
            for i in range(12):
                AA = chal_gd[:,i] 
                BB = chal_pred[:,i]
                print sklearn.metrics.roc_auc_score(AA[AA != -1],BB[AA != -1]),
            ...
            if epoch >= 2:
                ...
                test = test.set_value(test.index,
                                                      predict1,
                                                      test[predict1] + np.array(test_pred)/5)
                chal = chal.set_value(chal.index,
                                                      predict1,
                                                      chal[predict1] + np.array(chal_pred)/5)
                print ('ENSEMBLE',len(test_cost),sklearn.metrics.roc_auc_score(test[target1],test['predict']))
