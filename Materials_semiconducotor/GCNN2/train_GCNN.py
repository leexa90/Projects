...
train_bond = np.load('../train_GCNN.npy').item()
test_bond = np.load('../test_GCNN.npy').item()
train = pd.read_csv('../model_train_L2_0.04897.csv').sort_values('id').reset_index(drop=True)
test = pd.read_csv('../model_test_L2_0.04897.csv')
...
train['predict1'] = 0
test['predict1'] = 0
train['extra'] = train['predict2_0.04897']
test['extra'] = test['predict2_0.04897']
...
extra_feat = tf.placeholder(tf.float32, [1,1]) #3 atoms-wise features
P_matrix = tf.placeholder(tf.float32, [1,None,None,1]) # distance, Pair-wise features 
num_atoms =  tf.placeholder(tf.int32,None)
test[target1] = 0
### WEAVE MODULES ###
...
dense2 = tf.concat([meanA2,extra_feat],-1)
dense3 = tf.concat([tf.layers.dense(dense2,256,tf.nn.relu),extra_feat],-1)
dense4 = tf.concat([tf.layers.dense(dense3,32,tf.nn.relu),extra_feat],-1)
dense5 = tf.layers.dense(dense4,1,None)
# Evaluation
answer = tf.placeholder(tf.float32,[1,1],name='answer')
cost = tf.reduce_mean((dense5-answer )**2)
    ...
    for epoch in range(20):
        ...
        for sample in  shuffle:
            ...
            _, c = sess.run([optimizer, cost], feed_dict={Adj_matrix: [x1],
                                                          ...
                                                          learning_rate : lr,
                                                          extra_feat : [[x5]] })
        print (lr),    #print sess.run([extra_feat,extra_feat1], feed_dict={D_input: X,answer:[y], learning_rate : lr, extra_feat:z })
