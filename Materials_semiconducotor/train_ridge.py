...
if True:
    ...
    train[['id',name1,name2,target1,target2]].to_csv('modelR_train_%s.csv'%np.round((a+b)/2,5),index=0)
    test[['id',name1,name2]].to_csv('modelR_test_%s.csv'%np.round((a+b)/2,5),index=0)
die
