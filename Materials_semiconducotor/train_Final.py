'''
This is leexa90 code for kaggle competition.
Competition aims to predict quantum properties of certain semiconductor
materials, namely Al,In,Ga oxides in varying compositions

### Nomad2018 Predicting Transparent Conductors ###
https://www.kaggle.com/c/nomad2018-predict-transparent-conductors

This is an xgboost code, using 350 features of

Crystal lattice properties (vol,angles,)
elemental properties
bond lengths
bond/dihedral angles
forces and energy (vdw)
Linear estimate using above parameters

Subsampling of col ~ 20% and datapoints 40% was heavily done since it
was found to improve performance. Intuitively the linear estimate gets
the answer mostly correct, and the other 350 features provide incremental
predictions in the gradient boosting phase. Predictions was made using 20 fold CV,
with 1validation and 1 test set. 
...
if True:
    ...
    train2 = pd.read_csv('INPUT_FILES/train_data_features2.csv')
    test = pd.read_csv('INPUT_FILES/test.csv')
    test2 = pd.read_csv('INPUT_FILES/test_data_features2.csv')
    train2['id'] = train2['5']
    ...
    train_angle = np.load('INPUT_FILES/train_bond_angles.npy').item()
    test_angle = np.load('INPUT_FILES/test_bond_angles.npy').item()
    ...
    train_bond = np.load('INPUT_FILES/train_bond_distribution.npy').item()
    test_bond = np.load('INPUT_FILES/test_bond_distribution.npy').item()
    ...
    train_dihe = np.load('INPUT_FILES/train_dihe.npy').item()
    test_dihe = np.load('INPUT_FILES/test_dihe.npy').item()
    ...
    train_E = np.load('INPUT_FILES/train_energy.npy').item()
    test_E = np.load('INPUT_FILES/test_energy.npy').item()
    ...
    if True:
        ...
        a = pd.read_csv('INPUT_FILES/model_train_removeCorr_0.04958.csv').sort_values('id').reset_index(drop=True)['predict1_0.04958']
        b = pd.read_csv('INPUT_FILES/model_train_removeCorrLess_0.04969.csv').sort_values('id').reset_index(drop=True)['predict1_0.04969']
        train['predict1_0.495'] = (a + b)/2
        a = pd.read_csv('INPUT_FILES/model_train_removeCorr_0.04958.csv').sort_values('id').reset_index(drop=True)['predict2_0.04958']
        b = pd.read_csv('INPUT_FILES/model_train_removeCorrLess_0.04969.csv').sort_values('id').reset_index(drop=True)['predict2_0.04969']
        train['predict2_0.495'] = (a + b)/2
        a = pd.read_csv('INPUT_FILES/model_test_removeCorr_0.04958.csv').sort_values('id').reset_index(drop=True)['predict1_0.04958']
        b = pd.read_csv('INPUT_FILES/model_test_removeCorrLess_0.04969.csv').sort_values('id').reset_index(drop=True)['predict1_0.04969']
        test['predict1_0.495'] = (a + b)/2
        a = pd.read_csv('INPUT_FILES/model_test_removeCorr_0.04958.csv').sort_values('id').reset_index(drop=True)['predict2_0.04958']
        b = pd.read_csv('INPUT_FILES/model_test_removeCorrLess_0.04969.csv').sort_values('id').reset_index(drop=True)['predict2_0.04969']
        test['predict2_0.495'] = (a + b)/2


    if False:
        print '###doing eswald features###'
        train_E = np.load('INPUT_FILES/train_ewald_sum_data.npy.zip')['train_ewald_sum_data.npy'].item()
        train['Eswald_array'] = train['id'].apply(lambda x : train_E[x-1])
        test_E = np.load('INPUT_FILES/test_ewald_sum_data.npy.zip')['test_ewald_sum_data.npy'].item()
    ...
    for repeat in range(0,1):
        ...
        for seed in seeds:
            ...
            for i in range(0,20):
                test_id = [x for x in range(0,2400) if x%20 == i]
                train_id_ori = [x for x in range(0,2400) if x%20 != i] 
                for ii in range(0,19):
                    val_id = [x for x in train_id_ori if x%19 == ii]
                    train_id = [x for x in train_id_ori if x%19 != ii] 
                    ...
                    X_val, y_val1,y_val2 = train.iloc[val_id][cols], train.iloc[val_id][target1],\
                                                train.iloc[val_id][target2]
                    ...
                    xgval1 = xgb.DMatrix(X_val[cols].values, label=y_val1.values,missing=np.NAN,feature_names=cols)
                    watchlist  = [ (xgtrain,'train'),(xgval,'test2'),(xgval1,'test')]
                    model1_a = {}
                    model1=xgb.train(plst,xgtrain,1200,watchlist,early_stopping_rounds=200,
                                     evals_result=model1_a,maximize=False,verbose_eval=100)
                    hist1[0] += map(np.float,model1_a['test']['rmse'])
                    hist1[1] += map(np.float,model1_a['test2']['rmse'])
                    ...
                    X_val, y_val1,y_val2 = train.iloc[val_id][cols], train.iloc[val_id][target1],\
                                                train.iloc[val_id][target2]
                    train = train.set_value(test_id,'predict1',train.iloc[test_id]['predict1']+model1.predict(xgval, ntree_limit=model1.best_ntree_limit)/(19*len(seeds)))
                    test = test.set_value(test.index, target1, test[target1]+model1.predict(xgtest, ntree_limit=model1.best_ntree_limit)/(19*20*len(seeds)))
                    ...
                    xgval1 = xgb.DMatrix(X_val[cols].values, label=y_val2.values,missing=np.NAN,feature_names=cols)
                    watchlist  = [ (xgtrain,'train'),(xgval,'test2'),(xgval1,'test')]
                    model1_a = {}
                    params["eta"] = 0.06
                    params["max_depth"] = 8
                    if seed %2 == 0:
                        None#params["eta"] = params["eta"]/3
                    plst = list(params.items())
                    model2=xgb.train(plst,xgtrain,1400,watchlist,early_stopping_rounds=200,
                                     evals_result=model1_a,maximize=False,verbose_eval=100)
                    hist2[0] += map(np.float,model1_a['test']['rmse'])
                    hist2[1] += map(np.float,model1_a['test2']['rmse'])
                    train = train.set_value(test_id,'predict2',train.iloc[test_id]['predict2']+model2.predict(xgval, ntree_limit=model2.best_ntree_limit)/(19*len(seeds)))
                    test = test.set_value(test.index, target2, test[target2]+model2.predict(xgtest, ntree_limit=model2.best_ntree_limit)/(19*20*len(seeds)))
