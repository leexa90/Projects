...
for seed in seeds:
    ...
    for i in range(0,10):
        ...
        xgb = RandomForestRegressor(verbose=1)
        xgb = RandomForestRegressor(verbose=0,n_estimators=80,min_samples_leaf=25,min_samples_split=26)
        model1=xgb.fit(X_train[cols].values, y_train1.values)
        pred = model1.predict(X_test[cols].values)
        print np.mean((model1.predict(X_train[cols].values) - y_train1.values)**2)**.5,
        print np.mean(( pred- y_test1.values)**2)**.5
        train = train.set_value(test_id,'predict1',train.iloc[test_id]['predict1']+pred/len(seeds))
        test = test.set_value(test.index, target1, test[target1]+model1.predict(test[cols])/(10*len(seeds)))
        dictt_cols1[len(dictt_cols1.keys())+1] = model1.feature_importances_

        xgb = RandomForestRegressor(verbose=1)
        xgb = RandomForestRegressor(verbose=0,n_estimators=80,min_samples_leaf=25,min_samples_split=26)
        model1=xgb.fit(X_train[cols].values, y_train2.values)
        pred = model1.predict(X_test[cols].values)
        print np.mean((model1.predict(X_train[cols].values) - y_train2.values)**2)**.5,
        print np.mean(( pred- y_test2.values)**2)**.5    

        train = train.set_value(test_id,'predict2',train.iloc[test_id]['predict2']+pred/len(seeds))
        test = test.set_value(test.index, target2, test[target2]+model2.predict(test[cols])/(10*len(seeds)))
        dictt_cols2[len(dictt_cols2.keys())+1] = dictt_cols2[1].map(model2.feature_importances_)
