...
comps=1
def maximum_likelihood_feature(train,test,feature,train_id,final=100):
...
for seed in seeds:
    ...
    for i in range(0,10):
        ...
        impt = ['dihe_std','dihe_75',"N('O', 'O', 1)","N('O', 'O', 0)",
                ...
                "('O', 'O')_A_mean"]
        if True:
            x1,trn1,tst1,final1 = maximum_likelihood_feature(train,test,'CNN1',train_id,10)
            x2,trn2,tst2,final2 = maximum_likelihood_feature(train,test,'CNN2',train_id,10)
            train['temp1'] = trn1
            train['temp2'] = trn2
            test['temp1'] = tst1
            test['temp2'] = tst2
            train['CNN3'] = map(lambda x : (x[0],x[1]), train[['temp1',  'temp2']].values)
            test['CNN3'] = map(lambda x : (x[0],x[1]), test[['temp1',  'temp2']].values)
            a=train.iloc[train_id].groupby(['temp1',  'temp2'])[target1].apply(np.mean)
            b=train.iloc[train_id].groupby(['temp1',  'temp2'])[target1].apply(len)
            train['CNN3_std1'] = train['CNN3'].map(a.to_dict())
            train['CNN3_len1'] = train['CNN3'].map(b.to_dict())
            test['CNN3_std1'] = test['CNN3'].map(a.to_dict())
            test['CNN3_len1'] = test['CNN3'].map(b.to_dict())
            a=train.iloc[train_id].groupby(['temp1',  'temp2'])[target2].apply(np.mean)
            b=train.iloc[train_id].groupby(['temp1',  'temp2'])[target2].apply(len)
            train['CNN3_std2'] = train['CNN3'].map(a.to_dict())
            test['CNN3_std2'] = test['CNN3'].map(a.to_dict())
            names += ['CNN3_std1','CNN3_len1','CNN3_std2',]
##        for i in impt:
##            if i in train.keys():
##                x,trn,tst,final = maximum_likelihood_feature(train,test,i,train_id)
##                train['ML_'+i] = trn
##                test['ML_'+i] = tst
##                a=train.iloc[train_id].groupby('ML_'+i)[target1].apply(np.std)
##                b=train.iloc[train_id].groupby('ML_'+i)[target1].apply(len)
##                train['ML1_std_'+i] = train['ML_'+i].map(a.to_dict())
##                test['ML1_std_'+i] = test['ML_'+i].map(a.to_dict())
##                train['ML1_L_'+i] = train['ML_'+i].map(b.to_dict())
##                test['ML1_L_'+i] = test['ML_'+i].map(b.to_dict())
##                a=train.iloc[train_id].groupby('ML_'+i)[target2].apply(np.std)
##                b=train.iloc[train_id].groupby('ML_'+i)[target2].apply(len)
##                train['ML2_std_'+i] = train['ML_'+i].map(a.to_dict())
##                test['ML2_std_'+i] = test['ML_'+i].map(a.to_dict())
##                train['ML2_L_'+i] = train['ML_'+i].map(b.to_dict())
##                test['ML2_L_'+i] = test['ML_'+i].map(b.to_dict())
##                names += ['ML1_std_'+i,'ML2_std_'+i,'ML_'+i,'ML1_L_'+i,'ML2_L_'+i]
                #del test['ML_'+i],train['ML_'+i]
...
impt = ['dihe_std','dihe_75',"N('O', 'O', 1)","N('O', 'O', 0)","lattice_angle_alpha_degree","dihe_25","dihe_88.0_90.0","N('O', 'O', 2)","lattice_vector_3_ang"]
