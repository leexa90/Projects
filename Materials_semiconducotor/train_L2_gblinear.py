...
comps=len(cols)
regr = PCA(n_components=comps,whiten=True)
names = []
...
for name in names :
    train[name] = 0
    test[name] = 0
temp = regr.fit(pd.concat([train,test])[cols].values).transform(train[cols].values)
train = train.set_value(train.index,names,temp);
temp = regr.fit(pd.concat([train,test])[cols].values).transform(test[cols].values)
test = test.set_value(test.index,names,temp)
cols = names + ['predict1_0.495','predict2_0.495']
for seed in seeds:
    ...
    for i in range(0,10):
        ...
        model1_a = {}
        model1=xgb.train(plst,xgtrain,8000,watchlist,early_stopping_rounds=200,
        ...
        if seed %2 == 0:
            None#params["eta"] = params["eta"]/3
        plst = list(params.items())
        model2=xgb.train(plst,xgtrain,9500,watchlist,early_stopping_rounds=200,
                         evals_result=model1_a,maximize=False,verbose_eval=1000)
